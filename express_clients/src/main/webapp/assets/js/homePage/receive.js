    ...
    $.ajax({
        ...
        success: function (res) {
            if(res.stateCode == 100){
                $('#deliveryCom').append('<option style="display: none">--请选择快递公司--</option>');
...
$('#deliveryNo').bind('input propertychange', function() {
    if($('#deliveryNo').val() == ''){
        $('.button-small').css({'background-color': '#ecedef','color': '#c1c2c6'});
        $('.button-small').unbind('click');
    }else{
...
$('#sure').click(function () {
        $.ajax({
            type: "POST",
            url: "../receiveOrder/saveToMyOrder",
            contentType: "application/json; charset=utf-8",
            headers: {
                "Accept": "application/json; charset=utf-8",
                "Authorization": "Basic " + sessionStorage .getItem("token")
            ...
            data: JSON.stringify({
                goodsType: $('#type').find('option:selected').attr('class'),
                desc: $('#introduce').val(),
                deliverCompanyId: $('#deliveryCom').find('option:selected').attr('id'),
