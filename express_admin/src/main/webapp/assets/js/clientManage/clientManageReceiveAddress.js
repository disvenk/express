...
layui.use(['table', 'layer','form'], function () {
    ...
    table.render({
        ...
        , url: '../customer/ExpReceiveAddressPageList'
        , height: 471
        , cellMinWidth: 80
        , where:{id:id}
        , request: {
        ...
        , cols: [[
            {field: 'name', sort: true, title: '收件人',  align: 'center'}
            , {field: 'tel', sort: true, title: '收件人手机', align: 'center'}
            , {field: 'address', title: '收件人地址',width: 500, align: 'center'}
    ...
    table.on('tool(dateTable)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
       ...
       if (layEvent === 'edit') {
           $.ajax({
               type: 'POST',
               url: '../customer/ExpReceiveAddressDefault',
               dataType: 'json',
