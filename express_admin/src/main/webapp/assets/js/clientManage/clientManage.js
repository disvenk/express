...
layui.use(['table', 'layer','form'], function () {
    ...
    table.render({
        ...
        , cols: [[
            {field: 'icon',title: '头像',width: 120,align: 'center',templet:'#showImg', event: 'setSign', style:'cursor: pointer;'}
            , {field: 'userCode', sort: true, title: '用户名', align: 'center'}
            , {field: 'userMobile', title: '手机号', align: 'center'}
            , {field: 'userNickname', title: '昵称', align: 'center'}
            , {field: 'finalLoginDate', title: '最后登录时间',width: 177, align: 'center'}
    ...
    table.on('tool(dateTable)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'receiveDetail') {//收件地址
            location.href = 'clientManageReceiveAddressHtml?id='+data.id;
        } else if (layEvent === 'sendDetail') {
            location.href = 'clientManageAddressHtml?id='+data.id;
            ...
            location.href = 'clientManageEditHtml?id='+data.id;
...
layui.use(['form','laydate','layer'], function () {
    ...
    form.on('submit(formDemo)', function(data){
        ...
        table.render({
            ...
            , height: 471
            , cellMinWidth: 80
            , where:{name:name,userMobile:userMobile,schoolId:schoolId,isDelete:isDelete}
            , request: {
                pageName: 'pageNum' //页码的参数名称，默认：page
                , limitName: 'pageSize' //每页数据量的参数名，默认：limit
            }
