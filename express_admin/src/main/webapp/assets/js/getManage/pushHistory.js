...
layui.use(['table', 'layer','form'], function () {
    ...
    table.render({
        ...
        , url: '../receive/expNoticeMessagePageList'
        , height: 471
        , cellMinWidth: 80
        , request: {
        ...
        , cols: [[
              {field: 'deliveryNo',  title: '快递单号', align: 'center'}
            , {field: 'deliverName',  title: '快递公司', align: 'center'}
            , {field: 'sendTime', title: '推送时间',width: 177, align: 'center'}
            , {field: 'validateCode', title: '验证码', align: 'center'}
            , {field: 'userCode', title: '用户名', align: 'center'}
            , {field: 'userMobile', title: '用户手机号', align: 'center'}
            , {field: 'sendContent',  title: '推送内容', align: 'center'}
            , {field: 'schoolName',  title: '所属网点', align: 'center'}
            , {field: 'sendResult',  title: '推送结果', align: 'center'}
    ...
    table.on('tool(dateTable)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'push') {
            ...
            $.ajax({
                ...
                data: JSON.stringify({userId: data.userId,content:data.sendContent}),
                success: function (res) {
                    if(res.stateCode == 100){
                        var sendResult = '';
                        if(res.data.status == false){
                            sendResult = '失败';
                            layer.alert('推送失败', {icon: 0});
                        }
                        if(res.data.status == true){
                            sendResult = '成功';
                            layer.alert('推送成功', {icon: 1});
                        }
                        $.ajax({
                            type: 'POST',
                            url: '../receive/updateExpNoticeMessage',
                            dataType: 'json',
                            contentType: 'application/json; charset=utf-8',
                            headers: {
                                'Accept': 'application/json; charset=utf-8',
                            ...
                            data: JSON.stringify({id: data.id,sendResult:sendResult}),
                            success: function (res) {
                                setTimeout(function () {
...
layui.use(['form','laydate','layer'], function () {
    ...
    form.on('submit(formDemo)', function(data){
        ...
        table.render({
            ...
            , height: 471
            , cellMinWidth: 80
            , where:{schoolId:schoolId,userCode:userCode,userMobile:userMobile}
            , request: {
                pageName: 'pageNum' //页码的参数名称，默认：page
                , limitName: 'pageSize' //每页数据量的参数名，默认：limit
            }
            , cols: [[
                ...
                , {field: 'deliverName', sort: true, title: '快递公司', align: 'center'}
                , {field: 'sendTime', title: '推送时间',width: 177, align: 'center'}
                , {field: 'validateCode', title: '验证码', align: 'center'}
                , {field: 'userCode', title: '用户名', align: 'center'}
                , {field: 'userMobile', sort: true, title: '用户手机号', align: 'center'}
                , {field: 'sendContent', sort: true, title: '推送内容', align: 'center'}
