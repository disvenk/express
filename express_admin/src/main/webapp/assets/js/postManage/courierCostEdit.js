            ...
            success: function (res) {
                if(res.stateCode == 100){
                    ...
                    for(var i = 0;i<$('#type').find('option').length;i++){
                        if($('#type').find('option').eq(i).attr('id') == res.data.type){
                            $('#type').find('option').eq(i).attr('selected',true);
                            form.render();
                        }
                    }
                }else{
...
layui.use(['form','laydate','layer'], function () {
    ...
    form.on('submit(formDemo)', function(data){
        ...
        var fee = $('#fee').val();
        if(fee == null || fee == ''){
            layer.alert('请输入上门费', {icon: 0});
            return false;
        }

        if(type == null || type == undefined){
            layer.alert('请选择上门费类型', {icon: 0});
            return false;
        }
